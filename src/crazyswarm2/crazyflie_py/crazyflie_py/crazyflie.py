...
from crazyflie_interfaces.msg import TrajectoryPolynomialPiece, FullState, Position, AngVel
...
class Crazyflie:
    ...
    def __init__(self, node, cfname, paramTypeDict):
        ...
        self.cmdVelMsg.header.frame_id = "/world"

        self.cmdPositionPublisher = node.create_publisher(
            Position, f"{prefix}/cmd_position", 1
        )
    ...
    def cmdVel(self, roll_rate, pitch_rate, yaw_rate, thrust):
        ...
        """
        self.cmdVelMsg.header.stamp = self.node.get_clock().now().to_msg()
        self.cmdVelMsg.roll_rate = roll_rate
        self.cmdVelMsg.pitch_rate = pitch_rate
