...
int main(int argc, char **argv)
{
    ...
    RCLCPP_INFO(node->get_logger(), "Connecting to %s at %s", motionCaptureType.c_str(), motionCaptureHostname.c_str());
    libmotioncapture::MotionCapture *mocap = libmotioncapture::MotionCapture::connect(motionCaptureType, cfg);
    RCLCPP_INFO(node->get_logger(), "Connection established");
    ...
    RCLCPP_INFO(node->get_logger(), "Starting tracking loop");
    ...
    if (logClouds) {
      pointCloudLogger.log(timestamp/1000, markers);
        ...
        RCLCPP_DEBUG_THROTTLE(node->get_logger(), *node->get_clock(), 5000, "Tracking %lu rigid bodies", markers->size());
