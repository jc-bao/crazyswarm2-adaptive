...
//   DEBUG_PRINT("controllerPid\n");
  ...
  if (RATE_DO_EXECUTE(ATTITUDE_RATE, tick)) {
    ...
    // NOTE: the yaw, pitch, raw are in deg/s
  ...
  // Modify by Chaoyi: use force-torque controller
  // Original: roll, pitch, yaw, thrust
  // After: thrustSi, torque, torqueX, torqueY, torqueZ
